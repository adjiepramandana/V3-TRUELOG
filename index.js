...
async function initializeApp() {
  try {
    await BotToken.sync({ alter: true });
    console.log('✅ Database connected');

    const config = await BotToken.findOne();
    if (config) {
      botToken = config.token;
      userIdOwner = config.userIdOwner;
      if (botToken) startBot(botToken);
...
app.post('/setToken', async (req, res) => {
  ...
  try {
    await BotToken.destroy({ where: {} });
    await BotToken.create({ token });
...
app.post('/setId', async (req, res) => {
  ...
  try {
    let record = await BotToken.findOne();
    if (!record) {
      record = await BotToken.create({ userIdOwner: String(userId) });
    } else {
      await record.update({ userIdOwner: String(userId) });
    }

    userIdOwner = String(userId);
    res.json({ 
      status: 'success', 
    ...
    res.status(500).json({ status: 'error', message: 'Failed to save User ID' });
  }
}); 
...
async function restartApp() {
  ...
  try {
    ...
    console.error('Failed to save state before restart:', err);
  }
